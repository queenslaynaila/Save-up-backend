...
export default (router: Router) => {
  router.post('/', async (req, res) => {
    ...
    const SQL_CREATE_ADMIN = sql<typeof validationResult.data, Admin>(query);

    const newUser = await SQL_CREATE_ADMIN({first_name,last_name,phone_number,password:password_hash,role,}).one();
