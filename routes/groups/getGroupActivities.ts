                ...
                jsonb_build_object(
                    ...
                    'recipients', COALESCE(
                        jsonb_agg(jsonb_build_object(
                            'id', debit_recipients.recipient_id,
                            'name', recipient_details.full_name,
                            'amount', debit_recipients.amount
                        )), '[]'::JSONB
                    )
                )::JSONB AS metadata,
                debit_requests.created_at
            FROM debit_requests
            LEFT JOIN debit_recipients 
                ON debit_recipients.group_id = debit_requests.group_id
                AND debit_recipients.request_id = debit_requests.xid
            LEFT JOIN user_contact_details AS recipient_details 
                ON recipient_details.id = debit_recipients.recipient_id
            JOIN user_contact_details 
