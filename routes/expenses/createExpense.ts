...
const SQL_CREATE_EXPENSES = sql<z.infer<typeof expenseSchema>, ExtendedExpenseInterface>(`INSERT INTO expenses (description, category_id, amount, date, user_id)
...
export default (router: Router) => {
  router.post('/', authMiddleware(), async (req, res) => {
    ...
    const expense = await SQL_CREATE_EXPENSES({description,category_id,amount,date,user_id: loggedInUserId,}).one().catch(() => {
