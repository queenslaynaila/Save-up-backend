...
const SQL_UPDATE_EXPENSE = (updated: string) => sql<z.infer<typeof updateExpenseSchema> & { id: string }, ExtendedExpenseInterface>(updated);
...
export default (router: Router) => {
  router.patch('/:id', authMiddleware(), async (req, res) => {
    ...
    const values: z.infer<typeof updateExpenseSchema> & { user_id: string; expense_id: string ;id:string} = {
      user_id: userId,
    ...
    const result = await SQL_UPDATE_EXPENSE(updated)(values).one(new HttpError(400, 'Expense with given ID not found'));
