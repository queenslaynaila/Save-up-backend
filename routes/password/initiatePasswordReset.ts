...
export default (router: Router) => {
  router.post('/', async (req, res) => {
    ...
    if (
      securityResult.rows.length === 0 ||
      !(await bcrypt.compare(securityAnswer, securityResult.rows[0].answer))
    ) {
      return res
        .status(401)
        .json({ message: 'Incorrect security answer or security answer not found for the user' });
    }
