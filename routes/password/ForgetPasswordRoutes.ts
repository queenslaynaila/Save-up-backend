...
export const initiatePasswordReset = (router: Router) => {
  router.post<string, Record<string, never>, { message: string }, { phone_number: string }, Record<string, never>>(
    ...
    async (req, res) => {
      ...
      const token = await SQL_SAVE_TOKEN({ user_id: user.id, token: resetToken }).one(new HttpError(500, 'Error saving token'));
      const actualToken = token.token;
      sendSms(phone_number, `Your password reset token is: ${actualToken}. It expires in 10 minutes. Do not share with anyone.`);
