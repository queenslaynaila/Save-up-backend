...
{ user_id: string },
{ question: string; question_id: string }
>(
...
const SQL_CHECK_TOKEN_EXISTS = sql<
...
export const initiatePasswordReset = (router: Router) => {
  ...
  { phone_number: string },
  Record<string, never>
  >('/forget-password-request', async (req, res) => {
...
export const verifySecurityAnswers = (router: Router) => {
  ...
  Record<string, never>
  >('/verify-security-answers', async (req, res) => {
