...
export const createContributions = async (req: Request, res: Response) => {
  ...
  if (!validationResult.success) {
    return res.status(400).json({ error: new HttpError(400, 'Invalid saving id, amount, or date').message });
  }
...
export const updateContributions = async (req: Request, res: Response) => {
  ...
  if (!validationResultBody.success) {
    return res.status(400).json({ error: new HttpError(400, 'Invalid contributions data. Please provide valid values for all user fields.').message });
  }
