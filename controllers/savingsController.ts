...
export const updateUserTotalTargetedAmount = async (userId: string, newSavingTargetAmount: number) => {
    const query = `
        UPDATE users
...
export const createSaving = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const { user_id, description, category, target_amount, priority, target_date } = validatedSavings;
        const savingQuery = `
...
export const updateSaving = async (req: Request, res: Response) => {
    ...
    try {
        const validatedSavings = savingSchema.parse(req.body);
        const { description, category, target_amount, contributed_amount, priority, status, target_date, start_date } = validatedSavings;
        const query = `
            UPDATE savings 
            SET 
                description = $1,
            ...
            WHERE 
                id = $9 
            RETURNING *`;
        const values = [description, category, target_amount, contributed_amount, priority, status, target_date, start_date, id];
