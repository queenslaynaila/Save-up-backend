...
DECLARE
  ...
  v_reference_id                  INT;
BEGIN
  v_source_balance := get_transaction_info(p_user_id, p_source_pocket_id);
  ...
  v_destination_balance := get_transaction_info(p_user_id, p_destination_pocket_id);
...
GRANT EXECUTE ON FUNCTION create_transfer(INT, INT, INT, NUMERIC) TO app_user;
SELECT create_distributed_function(
  'create_transfer(INT, INT, INT, NUMERIC)', 'p_source_pocket_id'
);